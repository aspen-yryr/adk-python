  ...
  async def append_event(self, session: Session, event: Event) -> Event:
    ...
    with self.database_session_factory() as sql_session:
      ...
      assert storage_session is not None, (
          f"Session {session.id} not found in storage for app"
          f" {session.app_name} and user {session.user_id}."
      )
      ...
      if storage_app_state is None or storage_user_state is None:
        raise ValueError(
            f"App state or user state not found for app {session.app_name} and"
